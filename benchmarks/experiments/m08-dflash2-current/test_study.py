...
class StudyTests(unittest.TestCase):
    ...
    def test_candidate_preserves_other_serving_arguments(self):
        ...
        manifest = {'candidate_dtype': 'bfloat16', 'candidate': 'sha256:pinned',
                    'candidate_spec': {'method': 'dflash', 'model': '/draft', 'num_speculative_tokens': 7},
                    'draft_path': '/local/draft'}
        ...
        self.assertEqual(json.loads(cmd[cmd.index('--speculative-config')+1]), manifest['candidate_spec'])
        self.assertIn('/local/draft:/draft:ro', cmd)
