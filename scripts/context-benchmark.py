...
visible-content latency, completion timing, native vLLM phase counters, and raw
SSE evidence.

Adapted from the MIT-licensed Intel Arc Pro B70 inference cookbook at commit
...
def ensure_idle(root):
    ...
    for name in ("vllm:num_requests_running", "vllm:num_requests_waiting"):
        _, rows = metric_series(text, (name,))
        value = sum(rows.values())
        if value:
            raise RuntimeError(f"server is not idle: {name}={value}")
...
def request(url, root, model, messages, max_tokens, raw_path, cache_expected,
    ...
    record["native_decode_tokens_per_second"] = (
        ...
        if post_first_tokens and deltas["native_decode_seconds"] > 0 else None)
    return record
...
def main():
    ...
    native_prefill_values = [
        r["native_prefill_tokens_per_second"] for r in records
        if r["native_prefill_tokens_per_second"] is not None
    ]
    native_decode_values = [
        r["native_decode_tokens_per_second"] for r in records
        if r["native_decode_tokens_per_second"] is not None
    ]
    output = {
        "mode": args.mode,
        "budget": args.budget,
        ...
        "summary": {
            ...
            "native_prefill_tokens_per_second": stats(native_prefill_values),
            "native_decode_tokens_per_second": stats(native_decode_values),
