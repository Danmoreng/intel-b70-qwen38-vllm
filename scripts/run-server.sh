...
[[ "${SPECULATIVE_TOKENS:-6}" =~ ^[1-9][0-9]*$ ]] || { echo "SPECULATIVE_TOKENS must be positive" >&2; exit 2; }
[[ "${MAX_NUM_BATCHED_TOKENS:-6656}" =~ ^[1-9][0-9]*$ ]] || { echo "MAX_NUM_BATCHED_TOKENS must be positive" >&2; exit 2; }
...
vocab_mount=()
vocab_env=(-e B70_DRAFT_VOCAB_ENABLED=0)
if [[ -n "${DRAFT_VOCAB_FILE:-}" ]]; then
  [[ -f "$DRAFT_VOCAB_FILE" ]] || { echo "Draft vocabulary not found: $DRAFT_VOCAB_FILE" >&2; exit 2; }
  draft_vocab="$(realpath "$DRAFT_VOCAB_FILE")"
  vocab_mount=(-v "$draft_vocab:/opt/b70/draft-vocab.txt:ro")
  vocab_env=(-e B70_DRAFT_VOCAB_ENABLED=1 -e B70_DRAFT_VOCAB_PATH=/opt/b70/draft-vocab.txt)
fi

speculative_config="{\"method\":\"mtp\",\"num_speculative_tokens\":${SPECULATIVE_TOKENS:-6}}"
...
exec docker run --rm --name "$container" \
  ...
  --quantization gptq \
  --dtype float16 \
  --max-model-len "${CONTEXT_SIZE:-204800}" \
  --gpu-memory-utilization "${GPU_MEMORY_UTILIZATION:-0.93}" \
  --kv-cache-dtype fp8 \
  --max-num-seqs "${MAX_NUM_SEQS:-1}" \
  --max-num-batched-tokens "${MAX_NUM_BATCHED_TOKENS:-6656}" \
  "${cache_args[@]}" \
